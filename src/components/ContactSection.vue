    ...
    <div class="contact-section" id="contact-section">
        ...
        <div class="body">
            <div class="form-container">
                <form @submit.prevent>
                    <small :style="`color : ${alertText.type === 'error' ? 'red' : 'green'};`" v-if="alertText.content">{{alertText.content}}</small>
            ...
            <div class="map-container">
                ...
                <iframe :src="$store.state.mapLink" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async submitInput() {
                this.alertText.type = null;
                this.alertText.content = null;

                if(!this.name || !this.email || !this.phoneNumber || !this.message) {
                    this.alertText.type = 'error';
                    this.alertText.content = 'Please fill out all fields';
                } 
                else if(!(this.validateEmail(this.email) && this.validatePhoneNumber(this.phoneNumber))) {
                    this.alertText.type = 'error';
                    this.alertText.content = 'Please fill all fields correctly';
                }
                else {
                    try {
                        ...
                        const result = await axios.post('https://formsubmit.co/ajax/anmacexports@gmail.com', {
                            ...
                            submittedOn: this.currentDate
                        });
                        if(result.status == 200) {
                            this.alertText.type = 'success';
                            this.alertText.content = 'Details received! We will contact you shortly.';
                        }
                        else throw result;
                    } catch(error) {
                        this.alertText.type = 'error';
                        this.alertText.content = 'There was an error submitting your form. Please try again later, or contact us at the number provided below.';
                        console.log(error);
                    };
                };
            }
