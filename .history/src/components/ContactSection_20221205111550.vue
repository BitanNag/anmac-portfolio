...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async submitInput() {
                this.alertText = null
                if(!this.name || !this.email || !this.phoneNumber || !this.message) this.alertText = 'Please fill out all fields';
                else if(!(this.validateEmail(this.email) && this.validatePhoneNumber(this.phoneNumber))) this.alertText = 'Please fill all fields correctly';
                else {
                    try {
                        ...
                        if(result.status == 200) this.alertText
                        else throw result;
                    } catch(error) {
                        this.alertText = 'There was an error submitting your form. Please try again later, or contact us at the number provided below.';
                        console.log(error);
                    }
                }
            }
